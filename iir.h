 ...
 *
 * This file contains code covered by the following copyright
 * and license notice. The modifications to this file are licensed
...
// define this to float if you're brave and you'll see what happens...
#define gliirt float
...
void combine_iir_stages(int mode, iir_stage_t *gt, iir_stage_t *first,
                        iir_stage_t *second, int upf, int ups);
void free_iir_stage(iir_stage_t *gt);
void calc_2polebandpass(iirf_t *iirf, iir_stage_t *gt, float fc, float bw,
                        long sample_rate);

// for chebyshev we need iir stages with na=3, nb=2
// na are the forward coefficients
// nb are the recursive coefficients
int chebyshev(iirf_t *iirf, iir_stage_t *gt, int n, int mode, float fc,
              float pr);

/*
 * calculate butterworth coefficients
 * coefficient calculation taken from
 ...
 * r -> resonance
 */
static inline void butterworth_stage(
        iir_stage_t *gt, int mode, float f, float r, long sample_rate) {
...
/* process function */
static inline void iir_process_buffer(
        iirf_t *iirf, iir_stage_t *gt, const float *indata, float *outdata,
        const uint32_t numSampsToProcess, int add) {
    uint32_t pos;
        ...
        for (pos = 0; pos < numSampsToProcess; pos++) {
            ...
            for (i = 0; i < gt->nstages; i++) {
                ...
                for (j = gt->na; j < nt; j++) {
                    ...
                    iirf[i].oring[opos] += (
                        gt->coeff[i][j] * iirf[i].oring[z--]
                    );
                }
            }
            /* Doesn't matter if we process it in place */
...
static inline void iir_process_buffer_1s_5(iirf_t *iirf, iir_stage_t *gt,
        ...
        const uint32_t numSampsToProcess) {
    uint32_t pos;
    ...
    for (pos = 0; pos < numSampsToProcess; pos++) {
        ...
        /* y[n]=a0*x[n]+a1*x[n-1]+... */
        /* y[n]=y[n]+b1*y[n-1]+b2*y[n-2]+... */
        iirf[0].oring[2] = flush_to_zero(
            gt->coeff[0][0] * iirf[0].iring[2] +
...
/* process function */
static inline void iir_process_buffer_ns_5(
        iirf_t *iirf, iir_stage_t *gt, const float *indata, float *outdata,
        const uint32_t numSampsToProcess) {
    uint32_t pos;
    int i;
